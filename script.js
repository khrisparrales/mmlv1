...
function add(id_imdb) {
    $.getJSON(
        ...
        ids = response.release_date;
        console.log("la fecha: " + ids);
        document.getElementById("pt").innerHTML =
            "Elemento:" + e + " Pagina:" + f + " Total:" + pag;
...
function savePopup() {
    ...
    $.ajax(settings).done(function(response) {
        ...
        ids = response.release_date;
        var str = ids;
        var f = Number("20191030");
        var res = str.replace(/-/g, "");
        var s = url;
        var res2 = s.replace("view?usp=sharing", "preview");
        console.log(f + "-20191030");
        var obj = {
            id: Number(res),
            key: encodeURIComponent(response.title),
            name: response.title,
            description: response.overview,
            genres: [response.genres[0].name, response.genres[1].name],
            rate: response.vote_average,
            length: "1hr 48mins",
            img: "https://image.tmdb.org/t/p/w220_and_h330_face/" + response.poster_path,
            cover: "https://image.tmdb.org/t/p/original/" + response.backdrop_path,
            url: res2,
        };
